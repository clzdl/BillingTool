...
void TriggerSmsSendFile(ModuleContext *ctx, void *ptr)
{
	...
	std::string tmpInFile = CStringToString(ctx->m_funcGetProperty(MODULE_PROP, _TEXT("文件入口")), CP_ACP) + "/tmp_smsSendFile";
	CString csInFile = ctx->m_funcGetProperty(MODULE_PROP, _TEXT("文件入口"))
...
void TriggerOneWayStopMsg(ModuleContext *ctx, void *ptr)
{
	ListViewData resultViewData(ctx->m_funcGetProperty(0, _TEXT("测试号码")), _TEXT("触发单停短信"));
	resultViewData.m_result = _TEXT("触发成功.");
	...
	*/
	if (!BuildHastenNotice(ctx, ctx->m_funcGetProperty(0, _TEXT("用户ID")),
		_TEXT("3120"),
		ctx->m_funcGetProperty(MODULE_PROP, _TEXT("HASTEN_POLICY_ID")),
		ctx->m_funcGetProperty(MODULE_PROP, _TEXT("LEAVE_REAL_FEE")),
		ctx->m_funcGetProperty(MODULE_PROP, _TEXT("REAL_FEE")),
		ctx->m_funcGetProperty(MODULE_PROP, _TEXT("CREDIT_VALUE"))))
	{
		resultViewData.m_result = _TEXT("触发失败.");
	}
...
void TriggerDoubleStopMsg(ModuleContext *ctx, void *ptr)
{
	ListViewData resultViewData(ctx->m_funcGetProperty(0, _TEXT("测试号码")), _TEXT("触发双停短信"));
	resultViewData.m_result = _TEXT("触发成功.");
...
void TriggerNotEnoughBalanceMsg(ModuleContext *ctx, void *ptr)
{
	ListViewData resultViewData(ctx->m_funcGetProperty(0, _TEXT("测试号码")), _TEXT("触发余额不足短信"));
	resultViewData.m_result = _TEXT("触发成功.");
...
void TriggerDataRemindMsg(ModuleContext *ctx, void *ptr)
{
	ListViewData resultViewData(ctx->m_funcGetProperty(0, _TEXT("测试号码")), _TEXT("触发流量使用短信"));
	resultViewData.m_result = _TEXT("触发成功.");
...
void TriggerDataTopMsg(ModuleContext *ctx, void *ptr)
{
	ListViewData resultViewData(ctx->m_funcGetProperty(0, _TEXT("测试号码")), _TEXT("触发流量封顶短信"));
	resultViewData.m_result = _TEXT("触发成功.");
...
void TriggerPostPaypMsg(ModuleContext *ctx, void *ptr)
{
	ListViewData resultViewData(ctx->m_funcGetProperty(0, _TEXT("测试号码")), _TEXT("触发后付费用户催费短信"));
	resultViewData.m_result = _TEXT("触发成功.");
