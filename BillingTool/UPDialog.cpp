...
static UINT AUTO_INCREMENT_TIMER = 4444;
CUPDialog::CUPDialog(HWND hParentWnd,LP_CUPDIALOG_USERPROC lpUserProc,LPVOID lpUserProcParam,LPCTSTR lpszDlgTitle/*=_T("Please Wait..")*/,bool bAllowCancel/*=true*/, bool autoIncrement/*false*/)
{
...
INT_PTR CALLBACK ProgressDlgProc(HWND hDlg,UINT Message,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			if(pProgressDialog->m_bAutoIncrement)
				SetTimer(hDlg , AUTO_INCREMENT_TIMER, 500 , NULL);
			return TRUE;
		}
	case WM_COMMAND:
		{
	...
	case WM_TIMER:
		if (wParam == AUTO_INCREMENT_TIMER)
		{
			pProgressDialog = (CUPDialog*)GetWindowLongPtr(hDlg, GWLP_USERDATA);
			UINT maxPos = SendMessage(GetDlgItem(hDlg, pProgressDialog->m_nProgressBarControlId), PBM_GETRANGE, FALSE, 0);
			UINT pos = SendMessage(GetDlgItem(hDlg, pProgressDialog->m_nProgressBarControlId), PBM_GETPOS, 0, 0);
			if (pos >= maxPos)
				pos = 0;

			pos += 4;
			
			SendMessage(GetDlgItem(hDlg, pProgressDialog->m_nProgressBarControlId), PBM_SETPOS, pos, 0);
		}
		break;
	...
	case PROGRESSTHREADDATA::WM_CANCELPROGRESSTHREAD:			//Enough to Signal the Thread - Actual Handle Would be Closed in the Dialog Destructor
		{
			pProgressDialog = (CUPDialog*)GetWindowLongPtr(hDlg, GWLP_USERDATA);
			{
				...
				if (pProgressDialog->m_bAutoIncrement)
					KillTimer(hDlg,AUTO_INCREMENT_TIMER);
