...
void CSplashWindow::ShowSplashScreen(  CWnd* pParentWnd /*= NULL*/, LPCTSTR statusMessage /*= NULL*/, int millisecondsToDisplay /*= 0*/ )
{
	...
	// clear message queue
	MSG msg;
    while (m_pSplashWindow && IsWindow(m_pSplashWindow->m_hWnd) && PeekMessage(&msg, m_pSplashWindow->m_hWnd,  0, 0, PM_REMOVE)) {
...
BOOL CSplashWindow::Create(CWnd* pParentWnd /*= NULL*/)
{
	...
	BOOL result =  CreateEx(0,
		...
		NULL, WS_POPUP | WS_VISIBLE, xPos, yPos, width, height, NULL != pParentWnd? pParentWnd->GetSafeHwnd():NULL, NULL);
...
{	
	...
	CString nameStr;
	nameStr.Format(_TEXT("%s.EXE"), AfxGetApp()->m_pszExeName);
	...
	if (verSize != 0)
	{
		...
		LPVOID lplpBuffer;
		//080404b0  vs_version_info block header
		BOOL doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\080404b0\\ProductName"), &lplpBuffer, &ver);
		if ( doThis ) {
			wcsncpy_s(buf1, (TCHAR *)lplpBuffer, ver);
			m_productNameString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\080404b0\\CompanyName"), &lplpBuffer, &ver);
		if ( doThis ) {
			wcsncpy_s(buf1, (TCHAR *)lplpBuffer, ver);
			m_companyNameString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\080404b0\\LegalCopyright"), &lplpBuffer, &ver);
		if ( doThis ) {
			wcsncpy_s(buf1, (TCHAR *)lplpBuffer, ver);
			m_copyrightString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\080404b0\\Comments"), &lplpBuffer, &ver);
		if ( doThis ) {
			wcsncpy_s(buf1, (TCHAR *)lplpBuffer, ver);
			m_commentsString = (CString)buf1;
		}

		doThis = VerQueryValue (buf, TEXT("\\StringFileInfo\\080404b0\\ProductVersion"), &lplpBuffer, &ver);
		if ( doThis ) {
			wcsncpy_s(buf1, (TCHAR *)lplpBuffer, ver);
			...
			for ( int i=0; i<numberOfValues; i++ ) {
				...
				if ( !m_versionNumberString.IsEmpty() )
					m_versionNumberString += ".";
				number.Format(_TEXT("%d"), versionNumbers[i] );
				m_versionNumberString += number; 
			}
			m_versionString = _TEXT("Version ") + m_versionNumberString;
